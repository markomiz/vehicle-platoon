...
def simple_follow(num_cars=2):
    ...
    for i in range(num_cars):
        # Create a Vehicle
        initial_state = np.array([-i * 0.5 * 10, 0., 0., 10.])  # [x, y, theta, speed]
    ...
    for step in range(num_steps):
        ...
        for i, vs in enumerate(all_vehicle_systems):
            ...
            if len(vs.other_estimates) > 0 : ## if not lead car
                ...
                acc, steer = vs.controller.compute_control( all_vehicle_systems[i-1].vehicle.get_state(),  vs.estimator.state, timestep)
                vs.vehicle.noisy_single_track_update(steer, acc, timestep)
                vs.estimator.prediction(steer, acc, timestep)

                position_history[step, i+1, :] = vs.controller.desired_state[:2]
            else:
                vs.vehicle.noisy_single_track_update(steering_angle, acceleration, timestep)
                vs.estimator.prediction(steering_angle, acceleration, timestep)
