...
def create_test_road(num_steps):
...
def run_scenario(scenario="simple", num_steps=100,  num_cars=2, world=None, plot=True, mpc_horizon=1, v2v=True, network_loss=0.0):
    ...
    road_points, road_speeds = create_test_road(num_steps)
    ...
    for step in range(num_steps):
        ...
        for i,vs in enumerate(world.all_vehicle_systems): #
            ...
            if i > 0 and step >0:
                follow_dist = np.linalg.norm(position_history[step, i, :] - desired_history[step  , i, :])
                follow_dist_error_history[step -1, i - 1] = follow_dist
...
if __name__ == '__main__':
    ...
    run_scenario(scenario="predictive", v2v=True, num_cars=5, mpc_horizon=3, network_loss=0.00, num_steps=200)