...
class Controller:
    ...
    def calculate_desired_state(self, state_to_follow , current_state):
        ...
        desired_state = current_state * 1.0
        desired_state[3] = state_to_follow *1.0
        # get point on road ahead
        dist = self.desired_follow_time * self.current_state[3]
        desired_state[0] += dist * np.sin(desired_state[2])
        desired_state[1] += dist * np.cos(desired_state[2])
    ...
    def calculate_desired_states(self, states_to_follow, current_state ):
        ...
        for i, dist in enumerate(follow_distances):
            ...
            desired_states[i,1] -= dist * np.sin(current_state[2])
